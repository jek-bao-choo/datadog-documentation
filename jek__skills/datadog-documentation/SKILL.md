...
description: "Expert Datadog documentation navigator. Use when the user asks about: (1) Datadog Agent installation, configuration, host monitoring, infrastructure, platforms (Linux, Windows, macOS), containers (Kubernetes, Docker, ECS), serverless (AWS Lambda, Azure, GCP), network monitoring, integrations, IoT, fleet automation; (2) APM, tracing, SDK instrumentation, spans, traces, profiling, dynamic instrumentation, data streams, error tracking (backend), programming languages (Python, Java, Go, Node.js, .NET, PHP, Ruby, C++, Rust), OpenTelemetry; (3) Real User Monitoring (RUM), Session Replay, Product Analytics, Synthetic Testing, error tracking (frontend/mobile), web view tracking, browser/mobile instrumentation (JavaScript, Android, iOS, Flutter, React Native, Kotlin, Roku, Unity); (4) CloudPrem setup, deployment, operation, self-hosted Datadog; (5) logs, log collection, metrics, getting started guides."
---
